...
Scene *currentScene;
Scene *sceneList[7]; // array of diff scenes
...
void Update() {
   ...
   while (deltaTime >= FIXED_TIMESTEP) {
    ...
    // SCROLLING
    // only scroll if player is past a certain x and/or y point
    if (currentScene == sceneList[0]) {
    ...
    if (currentScene != sceneList[0] &&
        ...
        currentScene != sceneList[5] &&
        currentScene != sceneList[6]) {
        if (currentScene == sceneList[4] && currentScene->state.player->position.y < -34.5) { // stop vertical scrolling at a certain point
            viewMatrix = glm::translate(viewMatrix, glm::vec3(-currentScene->state.player->position.x, 34.45, 0));
        } else { // vertical and horizontal scrolling
            viewMatrix = glm::translate(viewMatrix, glm::vec3(-currentScene->state.player->position.x, -currentScene->state.player->position.y, 0));
        }
        
    }
    // if key has been obtained, show in inventory
    if (currentScene->state.objective == "exit") {
        currentScene->state.inventory->position = glm::vec3(currentScene->state.player->position.x - 1.25 , currentScene->state.player->position.y - 3.0, 0);
        currentScene->state.inventory->isActive = true;
    }
...
void Render() {
    ...
    currentScene->Render(&program, fontTextureID);
    ...
    if (currentScene != sceneList[0] &&
        ...
        if (currentScene == sceneList[4] && currentScene->state.player->position.y < -34.5) {
            Util::DrawText(&program, fontTextureID, "Level " + std::to_string(levelNum),
                     0.2f, 0, glm::vec3(currentScene->state.player->position.x - 4.5, -34.5 + 3.25 + 0.03, 0));
            
            Util::DrawText(&program, fontTextureID, "Lives: " + std::to_string(lives),
                     0.2f, 0, glm::vec3(currentScene->state.player->position.x - 4.5, -34.5 + 2.85 + 0.03, 0));
            ...
            Util::DrawText(&program, fontTextureID, "Lives: " + std::to_string(lives),
                     0.2f, 0, glm::vec3(currentScene->state.player->position.x - 4.5, currentScene->state.player->position.y + 2.8 + 0.03, 0));
            Util::DrawText(&program, fontTextureID, "Inventory: ",
                     0.3f, 0, glm::vec3(currentScene->state.player->position.x - 4.5, currentScene->state.player->position.y - 3.0, 0));
        }
